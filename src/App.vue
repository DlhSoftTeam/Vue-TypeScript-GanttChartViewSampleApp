<template>
  <div id="app">
    ...
    <GanttChart v-bind:items="items" v-bind:settings="settings"
...
export default class App extends Vue {
  ...
  constructor() {
    ...
    items[2].predecessors = [{ item: items[1] }]; // Task 2 depends on Task 1.
    items[7].predecessors = [{ item: items[6], dependencyType: 'StartStart' }]; // Task 6 depends on Task 5 using Start-Start dependency type.
    items[8].predecessors = [{ item: items[0] }, { item: items[3] }]; // Milestone depends on Story A and Story B.
    ...
    // Customize columns.
    const columns = DlhSoft.Controls.GanttChartView.getDefaultColumns(items, settings);
    const indexOffset = columns[0].isSelection ? 1 : 0;
    ...
    // columns[10 + indexOffset].cellTemplate = DlhSoft.Controls.GanttChartView.getAssignmentSelectorColumnTemplate(184, (item) { return ['Resource 1', 'Resource 2'] });
  ...
  onItemChanged(args: any) {
    const item = args.item, propertyName = args.propertyName, isDirect = args.isDirect, isFinal = args.isFinal;
